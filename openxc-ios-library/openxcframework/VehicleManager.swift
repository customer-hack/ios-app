...
//  }
  ...
  fileprivate func protobufDignosticMessage(msg : VehicleMessage){
    ...
    if rsp.value != nil {
      if rsp.pid != nil {
        vmlog("diag rsp msg:\(rsp.bus) id:\(rsp.message_id) mode:\(rsp.mode) pid:\(rsp.pid ?? 88) success:\(rsp.success) value:\(rsp.value ?? 55)")
      } else {
        vmlog("diag rsp msg:\(rsp.bus) id:\(rsp.message_id) mode:\(rsp.mode) success:\(rsp.success) value:\(rsp.value ?? 55)")
      }
    } else {
      if rsp.pid != nil {
        vmlog("diag rsp msg:\(rsp.bus) id:\(rsp.message_id) mode:\(rsp.mode) pid:\(rsp.pid ?? 33) success:\(rsp.success) payload:\(rsp.payload)")
      } else {
